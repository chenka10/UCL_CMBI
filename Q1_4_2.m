...
%% Example data (TE values and corresponding signal intensities)
TE = [10, 20, 30, 40, 50]; % Echo times (ms)
signal = [100, 80, 60, 40, 20]; % Signal intensities

[exp_tr,gof_tr] = fit(signal,TE,"exp2");

%% Estimate T2

b_zero_indices = (bvals==0);
Avox_bzero = Avox(b_zero_indices);
TE_bzero = TE(b_zero_indices);

design_matrix = [ones(numel(TE_bzero),1) -TE_bzero'];
A = log(Avox_bzero);

x = pinv(design_matrix)*A;

S0_approx = exp(x(1));
T2_approx = 1/x(2);

model_res = exp(design_matrix*x);

figure;
scatter(1:numel(model_res),Avox_bzero);
hold on;
scatter(1:numel(model_res),model_res);
